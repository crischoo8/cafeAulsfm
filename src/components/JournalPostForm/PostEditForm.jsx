...
import { updatePostService, uploadToS3Service,swalBasicSettings } from "../../utilities/posts-service";
import Swal from "sweetalert2";

const log = debug("cafeaulsfm:src:components:PostEditForm");
...
export default function PostEditForm({post, setPost}) {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      const newPost = await updatePostService(postID, {
        ...postData,
        images: imgURL,
      });
      ...
      if (prompt.isConfirmed) {
        const removedPost = post?.filter((item) => item._id !== postID);
        setPost([...removedPost, newPost]);
        navigate("/journal");
      }
    } catch (err) {
